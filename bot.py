...
async def cmd_start(ev):
    me = await client.get_me()
    await ev.reply(f"🌹 **RoseUserBot v6.0 (Replit)**\n👑 Logged in as: @{me.username or me.first_name}\n📘 Use `.help` to see all commands.")
...
async def cmd_help(ev):
    await ev.reply(
        "**RoseUserBot v6.0 — Commands**\n\n"
        "🎧 VC / Music\n"
        "`.joinvc <group_id>` — Join VC\n"
        "Reply to voice + `.play` — Play that voice\n"
        "`.playlink <YouTube_URL>` — Queue YouTube\n"
        "`.queue`, `.skip`, `.pause`, `.resume`, `.stop`, `.loop on|off`, `.leavevc`\n\n"
        "📩 Messaging\n"
        "`.senddm <id> <text>` | `.sendfile <id> <filename>` | `.bc <id>`\n\n"
        "🛡️ Admin\n"
        "`.kick @user` | `.ban @user` | `.unban @user` | `.invite @user`\n\n"
...
async def cmd_joinvc(ev):
    ...
    try:
        ...
        LAST_VC = gid
        save_vc(gid)
        await ev.reply(f"✅ Joined VC `{gid}`.\nNow reply to a voice with `.play` or use `.playlink`.")
    except Exception as e:
        await ev.reply(f"❌ Join VC failed: `{e}`")
...
if __name__ == "__main__":
    try:
        ...
        print("❌ Stopped manually.")from pytgcalls.types.input_stream import AudioPiped
from pytgcalls.types import AudioQuality
